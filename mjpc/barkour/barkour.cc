...
std::string Barkour::XmlPath() const  //! 获取world的配置文件
{
  return absl::StrCat(mjpc::GetExecutableDir(), "/../mjpc/barkour/world.xml");
}
...
void Barkour::ResidualFn::GetNormalizedFootTrajectory(
    double duty_ratio, 
    double gait_frequency, 
    double normalized_phase_offset,
    double time, 
    double amplitude_forward, 
    double amplitude_vertical,
    double* target_pos_x, 
    double* target_pos_z) const 
{
  // 步态周期长度
  double step_duration = 1.0 / fmax(gait_frequency, 0.0001);  
  // 根据占空比duty_ratio计算支撑相和摆动相的持续时间
  ...
  if (in_phase_time < 0.0) 
    in_phase_time += step_duration;
  ...
  // 根据当前阶段，计算足部在水平和垂直方向上的目标位置
  if (is_flight_phase) 
  { // 飞行阶段，则计算目标位置的垂直方向上的位移（*target_pos_z）和水平方向上的位移（*target_pos_x）
    *target_pos_z += mju_sin(normalized_phase * mjPI) * amplitude_vertical;
    *target_pos_x += mju_sin(normalized_phase * mjPI - mjPI / 2) * amplitude_forward;
  } 
  else 
  { // 站立阶段，则只计算目标位置的水平方向上的位移（*target_pos_x）
    *target_pos_x += -mju_sin(normalized_phase * mjPI - mjPI / 2) * amplitude_forward;
  }
}
                                   ...
                                   double* residual) const 
{
  ...
  // common info
  double* torso_xmat = data->xmat + 9 * torso_body_id_; // torso相关信息
  double* compos = SensorByName(model, data, "torso_subtreecom"); // center of mass of the torso
  CHECK(compos != nullptr);

  double pitch = atan2(-torso_xmat[6], mju_sqrt(torso_xmat[7] * torso_xmat[7] + torso_xmat[8] * torso_xmat[8]));
  double roll = atan2(torso_xmat[7], torso_xmat[8]);

  // 计算heading
  double torso_heading[2] = {torso_xmat[0], torso_xmat[3]};
  if (parameters_[target_body_pitch_id_] > 1.0 || parameters_[target_body_pitch_id_] < -1.0) 
  {
  ...
  double linvel_ego[3]; // robot frame下的线速度
  ...
  for (BkFoot foot : kFootAll) // 世界坐标系下的足端位置
  {
    foot_pos[foot] = data->site_xpos + 3 * foot_site_id_[foot];
  }

  double* shoulder_pos[kNumFoot];
  for (BkFoot foot : kFootAll)  // 世界坐标系下的肩部位置
    shoulder_pos[foot] = data->xpos + 3 * shoulder_body_id_[foot];

  // average foot position
  double avg_foot_pos[3];
  AverageFootPos(avg_foot_pos, foot_pos); // 计算足端位置的平均值avg_foot_pos

  // Residual Calculations
  //! ---------- BodyHeight ----------
  // quadrupedal or bipedal height of torso
  double* body = SensorByName(model, data, "body"); // real body height
  residual[counter++] = body[2] - parameters_[target_body_height_id_];  // target body height

  //! ---------- BodyXYPos ----------
  double* pos = SensorByName(model, data, "torso_subtreecom");  // real body pos
  CHECK(pos != nullptr);
  residual[counter++] = pos[0] - parameters_[goal_position_x_id_];  // target body x pos
  residual[counter++] = pos[1] - parameters_[goal_position_y_id_];  // target body y pos

  //! ---------- Heading ----------
  mju_normalize(torso_heading, 2);  // normalize the heading vector
  residual[counter++] = torso_heading[0] - mju_cos(parameters_[target_body_heading_id_]); // target heading x
  residual[counter++] = torso_heading[1] - mju_sin(parameters_[target_body_heading_id_]); // target heading y

  //! ---------- Body Roll and Pitch ----------
  residual[counter++] = pitch - parameters_[target_body_pitch_id_]; // target body pitch
  residual[counter++] = UnwindAngle(roll, current_base_roll_) - parameters_[target_body_roll_id_]; // target body roll
  residual[counter++] = 0;

  //! ---------- Forward Velocity ----------
  residual[counter++] = linvel_ego[0] - parameters_[target_forward_velocity_id_]; // target forward velocity

  //! ---------- Sideways Velocity ---------
  residual[counter++] = linvel_ego[1] - parameters_[target_sideways_velocity_id_]; // target sideways velocity

  //! ---------- Upward Velocity -----------
  residual[counter++] = linvel_ego[2] - parameters_[target_upward_velocity_id_]; // target upward velocity

  //! ---------- Roll Pitch Yaw Velocity ----------
  residual[counter++] = angvel[0] - parameters_[target_roll_velocity_id_];  // target roll velocity
  residual[counter++] = angvel[2] - parameters_[target_turning_velocity_id_];     // target yaw velocity
  ...
  for (int foot_id = 0; foot_id < 4; foot_id++) 
  {
    ...
    mju_sub3(shoulder_pos_local, shoulder_pos[foot_id], compos); // 肩部位置相对于compos的位置,相当于base坐标系下shoulder的位置
    RotateVectorAroundZaxis(shoulder_pos_local, -heading_angle, shoulder_yaw_aligned[foot_id]); // 肩部位置在yaw aligned坐标系下的位置

    double foot_pos_local[3];
    mju_sub3(foot_pos_local, foot_pos[foot_id], compos);  // 足端位置相对于compos的位置
    RotateVectorAroundZaxis(foot_pos_local, -heading_angle, foot_current_poses_yaw_aligned[foot_id]); // 足端位置在yaw aligned坐标系下的位置

    foot_current_poses_yaw_aligned[foot_id][2] = foot_pos[foot_id][2];  // z方向的位置不变，取世界坐标系下的z坐标
  }
  ...
  for (int dim = 0; dim < 3; dim++) 
  {
    for (int foot_id = 0; foot_id < 4; foot_id++) 
    {
      double foot_pos_delta = parameters_[dist_from_nominal_ids_[foot_id][dim]];  // 从nominal位置的偏移
      double foot_target_pos;
      if (dim < 2) 
      {
        if (foot_id >= 2 && dim == 1) 
        {
          foot_pos_delta *= -1; // y方向的偏移取反
        }
        foot_target_pos = shoulder_yaw_aligned[foot_id][dim] + kNominalFootFromShoulder[foot_id][dim] + foot_pos_delta; // 计算足端位置的目标位置
      } 
      else // dim == 2
      {
      ...
      // 计算足端位置在三轴方向上的残差
      residual[counter++] = foot_target_pos - foot_current_poses_yaw_aligned[foot_id][dim];
    }
  }
  ...
  for (int foot_id = 0; foot_id < 4; foot_id++) 
  {
    ...
    // 依次计算每个足端的target_pos_x和target_pos_z
    ...
    residual[counter++] = target_pos_z - foot_current_poses_yaw_aligned[foot_id][2];
    residual[counter++] = target_pos_x - foot_current_poses_yaw_aligned[foot_id][0];
  }

  //! ---------- Balance ----------
  double capture_point[3];
  double fall_time = mju_sqrt(2 * parameters_[target_body_height_id_] / 9.81);
  mju_addScl3(capture_point, compos, comvel, fall_time);  // 计算capture point = compos + comvel * fall_time
  ...
  for (BkFoot foot : kFootAll) 
  {
    ...
    {
      residual[counter + 3 * foot + joint] *= kJointPostureGain[joint]; // 乘以关节角度的增益
    }
  }

  // loosen the "hands" in Biped mode. Biped模式下不需要跟踪手部的关节角度
  ...
  if (model->na > 0) 
  {
    mju_copy(residual + counter, data->act_dot, model->na); // time-derivative of actuator activation 
    counter += model->na;
  } 
  else 
  {
...
void Barkour::TransitionLocked(mjModel* model, mjData* data) 
{
  ...
  data->mocap_pos[3 * goal_mocap_id_] = parameters[residual_.goal_position_x_id_];
  data->mocap_pos[3 * goal_mocap_id_ + 1] = parameters[residual_.goal_position_y_id_];
  ...
  residual_.current_base_roll_ = UnwindAngle(roll_angle, residual_.current_base_roll_);
}
...
void Barkour::ResidualFn::ResetLocked(const mjModel* model) 
{
  ...
  for (const char* footname : {"foot_front_left", "foot_hind_left", "foot_front_right", "foot_hind_right"}) 
  {
    int foot_id = mj_name2id(model, mjOBJ_SITE, footname);
    
    if (foot_id < 0) 
      mju_error_s("site '%s' not found", footname);
  ...
  // shoulder ids
  shoulder_body_id_[kFootFL] = mj_name2id(model, mjOBJ_BODY, "upper_leg_front_left");
  shoulder_body_id_[kFootHL] = mj_name2id(model, mjOBJ_BODY, "upper_leg_hind_left");
  shoulder_body_id_[kFootFR] = mj_name2id(model, mjOBJ_BODY, "upper_leg_front_right");
  shoulder_body_id_[kFootHR] = mj_name2id(model, mjOBJ_BODY, "upper_leg_hind_right");
  ...
  target_sideways_velocity_id_ = ParameterIndex(model, "TargetSidewaysVelocity");
  ...
  CHECK_GE(balance_ids_[kFootFL], 0); // 检查balance_ids_[kFootFL]是否大于等于0
  ...
  for (int i = 0; i < 12; i++) 
  { 
    target_joint_angle_ids_[i] = ParameterIndex(model, absl::StrCat("q", i+1)); // 12个关节位置
  }

  walker_root_joint_id_ = mj_name2id(model, mjOBJ_JOINT, kRootJointName); // get root id
...
void Barkour::ResidualFn::AverageFootPos(double avg_foot_pos[3], double* foot_pos[kNumFoot]) const 
{
  // we should compute the average foot position for feet that are used for balancing.
